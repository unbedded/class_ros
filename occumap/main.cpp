...
const double l0 = 0, locc = 0.4, lfree = -0.4;
...
void visualization()
{
    ...
    for (int c = 0; c < C; c++)
    {
        cout << "Remaining Rows= " << C - c << endl;
        for (int r = 0; r < R; r++)
        {
            const char *color =
                (l0 == l[c][r]) ? "g." :  //Green unkown state
                    (l[c][r] > 0) ? "k."  //Black occupied state
                                  : "r."; //Red free state
            plt::plot({(double)c}, {(double)r}, color);
        }
    }

    plt::save("./Images/OccuGrid.png");
    plt::clf();
}
